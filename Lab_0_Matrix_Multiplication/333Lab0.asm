...
###########################################################################
		...
		call MULT				# Call the multiplication function to initialize a7 (the number of elements in the matrix)
...
LOOP:
	...
	addi  t0, t0, 1
		...
		addi s2, s2, 4             #change s4 to store location <- (s2 (array start) + t1 (offset))
...
MULT:	
    addi sp, sp, -20		#Adjust stack pointer to make room for 5 registers (ra, t0, t1, t2, t3)
